<template>
<ul class="menu">
	<li v-for="(item,i) in aList" @click="fnSetMenu([i,-1])" v-bind:key=i>
		...
		<ul v-if="item.submenu.length>0" :class="(i==aNowMenu[0])?'current'+item.submenu.length:''">
			<li v-for="(subitem, j) in item.submenu" @click.stop="fnSetMenu([i,j])" v-bind:key=j><router-link :to="subitem.url"  :class="(j==aNowMenu[1])?'active':''"><i class="fa fa-circle-o"></i>{{subitem.name}}</router-link></li>
		</ul>
	</li>
</ul>
</template>
